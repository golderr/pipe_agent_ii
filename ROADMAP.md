...
| B.1 | Next.js scaffolding + Supabase + shadcn + auth + nav shell + one live read path | `in_progress` | 2026-04-24: Local Next.js scaffold, magic-link auth shell, route shell, minimal `/coverage` read path, and authenticated-read RLS migration are implemented. `npm run typecheck`, `npm run lint`, `npm run build`, and `npm audit --audit-level=moderate` pass. Local smoke: `/login` renders and unauthenticated `/coverage` redirects to `/login?next=%2Fcoverage`. Still needs Vercel deployment and real authenticated browser-session verification before marking done; this workspace currently has no Vercel CLI, `VERCEL_TOKEN`, `VERCEL_ORG_ID`, `VERCEL_PROJECT_ID`, `ALLOWED_EMAILS`, or `NEXT_PUBLIC_SITE_URL`. See `docs/ops/frontend_deployment.md`. Original scope: Next.js (app router), TypeScript, Tailwind, shadcn/ui, Supabase JS client + Supabase Auth (magic-link), email allowlist, unauthenticated redirect to `/login`, Mapbox GL JS via `react-map-gl`, cmdk, TanStack Query, Zustand, Lucide icons. Deploy to Vercel. Per-surface URL routing: `/coverage`, `/review`, `/pipeline`, `/dashboard`. RLS policies provisioned on day one - authenticated read on project/evidence/resolution_log/coverage tables; mutation tables locked to service role. B.1 must prove one end-to-end read path: an authenticated user loads `/coverage`, reads jurisdiction rows through Supabase PostgREST + RLS, and renders them. This `/coverage` route is deliberately minimal/unstyled in B.1; the full Coverage table, filters, pins, and source-detail UI are B.2. |
| B.2 | Coverage view | `in_progress` | 2026-04-24: Local Coverage view implemented with Supabase-backed jurisdiction/project/source/review-item aggregation, filter bar, session-sticky filters, local-only pins, optional columns, queue priority counts, source freshness, expandable source detail, and disabled Phase C actions. Added read-only authenticated RLS for `review_items` so Coverage can show queue/deferred counts without opening writes. `npm run typecheck`, `npm run lint`, `npm run build`, and `npm audit --audit-level=moderate` pass. Still needs real authenticated browser smoke after B.1 deployment credentials are configured. Refresh and Upload buttons remain disabled until Phase C. |
