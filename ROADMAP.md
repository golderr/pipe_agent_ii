...
| A.6 | Review resolution discrepancies, tune if needed | `in_progress` | 2026-04-23: Validation found and fixed three systematic issues before rerun: future CoStar delivery dates no longer act as row-level freshness for unrelated fields, Stalled/Inactive transitions are manual-review only, and the polluted `Category` developer registry row is ignored both for matching and persistence; fuzzy-review developer suggestions are flagged without becoming resolved values. Status review confirmed the remaining 9 status deltas do not include Stalled/Inactive transitions. Phase A review packets now live under `docs/notes/phase_a_review/`: `status_review.csv` (9), `units_review.csv` (27), `delivery_review.csv` (35 explicit overwrites), `delivery_estimate_spotcheck.csv` (10 sampled from 218 estimated fills), `developer_review.csv` (44 non-Category rows), plus `developer_category_cleanup.csv` (84 FYI rows) and alias/cluster helper outputs. Validation memo: `docs/notes/phase_a_validation.md`. |
| A.7 | Run `canonicalize-developers --market los_angeles` (shadow mode) | `done` | 2026-04-23: Shadow run completed with no writes. After deleting the polluted `Category` registry row and its aliases in production, the rerun scanned 318 registry rows and 967 projects with non-null developer values; would change 0 project values in this mode. Match summary: 1,134 exact, 2 fuzzy-auto, 13 fuzzy-review, 0 new registry-entry candidates. `--apply` now skips fuzzy-review registry merges and project developer rewrites so ambiguous matches remain shadow-only until reviewed. |
