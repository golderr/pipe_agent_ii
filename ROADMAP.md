...
| B.3 | Pipeline list view (table + map) | `done` | 2026-04-25: Implemented read-only Pipeline with Supabase-backed project/evidence/APN data, dense sortable table, collapsible filter sidebar, sticky filters, local saved views, row hover preview, keyboard navigation (`j`/`k`, Enter, `/`), command search, read-only preview drawer, and Mapbox GL map view using OpenStreetMap raster tiles with status-colored clustered pins. New Project remains disabled until Phase C. Verified with `npm run typecheck`, `npm run lint`, `npm run build`, `npm audit --audit-level=moderate`, and `pytest -q`. Deployed to production at `https://tcg-pipeline.vercel.app`; logged-out `/pipeline` redirects to `/login?next=%2Fpipeline`. |
