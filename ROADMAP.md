...
| D.3d | Extraction-time Pass 3a re-extraction | `not_started` | Conflict / low-confidence / parse-error / refusal triggers per §8.7. Match-triggered Pass 3b ships in D.4. Budget/design the status-field disagreement trigger with known Pass 1 status-phrase negation noise in mind so "not approved"/"not opposed" false positives do not create runaway Pass 3 calls. Depends on D.3c. |
...
| D.B | 8-week BizJournals backfill | `not_started` | `tcg-pipeline news backfill --source bizjournals_la --since <8wk>`. ~200 URLs, ~$10 cost per §17.3. Observable via Coverage queue depth. Benchmark Pass 1 dictionary scans during backfill and decide whether to cache Aho-Corasick automatons per worker/batch. Depends on D.6, D.M. |
