...
| C.c | Promote `researcher_override` to a table | `done` | 2026-04-26: Added `researcher_overrides` table migration and SQLAlchemy model with active `(project_id, field_name)` uniqueness, legacy JSONB backfill, authenticated read-only RLS, and no direct client write grants. Added shared override helpers that merge legacy JSONB with active table rows (table wins per field), warn on legacy-only divergence, preserve first-set audit metadata on reaffirm, and dual-write/prune the legacy JSONB during the transition so Phase B Overrides UI remains stable. Resolution now reads table-backed overrides first; C.i replaced silent supersession/clearance with contradiction review items. Review workflow writes overrides through the shared helper; developer canonicalization respects active table overrides. Legacy JSONB column is intentionally retained until table-backed write/read paths are verified in a deployed environment. |
| C.d | Inline editing on Core fields (Evidence-derived) | `done` | 2026-04-27: Implemented FastAPI set/clear override endpoints for evidence-derived Core fields (`pipeline_status`, unit counts, `developer`, `product_type`, `age_restriction`, `date_delivery`). Writes go through `researcher_overrides` with Supabase user IDs, keep legacy JSONB synchronized, re-run `resolve_project(apply=True)`, update project edit metadata, and write `change_log` rows with `researcher_override`. New manual Core edits use `mode = review_protected`, not sticky: the manual value remains current, but any newer auto-resolved value that differs for that field creates an `override_contradiction` review item. Project Detail Core rows now show an inline edit control and override info cue; Vercel preview server actions remain blocked by default unless `ENABLE_PREVIEW_WRITES=true` for an approved session. |
| C.e | Inline editing on Identity + Notes fields | `done` | 2026-04-27: Implemented FastAPI direct-field writes through `POST /projects/{project_id}/field` for researcher-authored identity/workflow fields (`project_name`, aliases/raw addresses, city/state/county/ZIP, region, source URLs, planner contacts, inclusion flags/note). Added append-only `project_notes` table with authenticated read-only RLS and `POST /projects/{project_id}/note`; note appends keep the legacy latest-note project columns synchronized for Project Detail. Project Detail now uses the same inline editor shell for Core overrides, direct Identity fields, and append-only Notes; direct field and note writes go through Next.js server actions with preview writes blocked by default. |
...
| 2026-04-27 | Identity fields write directly; notes append history | C.e keeps the write-boundary split explicit: evidence-derived Core edits remain researcher overrides, while researcher-authored Identity/workflow fields update the `projects` row directly through FastAPI. Project notes are append-only in `project_notes`; legacy note columns stay synchronized as latest-note display fields until read surfaces move fully to note history. Authenticated PostgREST clients receive SELECT only for `project_notes`, with writes reserved for the FastAPI service connection. |
