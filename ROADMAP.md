...
| C.i | Contradiction detection service | `not_started` | Per `docs/specs/review_workflow.md` §5 and `docs/specs/EVIDENCE_LAYER_DECISIONS.md` §22. C.d added the minimum resolver-triggered `override_contradiction` path for newer evidence against `review_protected` Core edits. C.i still needs the full service: post-ingest batching, review queue lifecycle/actions for contradiction rows, broader source/field handling, and retiring legacy `until_newer_evidence` supersession as load-bearing behavior. |
...
| 2026-04-27 | Core inline edits are review-protected, not sticky | Supersedes the C.d contradiction note above. Manual Core-field values remain current until reviewed, but they are not permanent locks: any newer auto-resolved value that differs from the manually edited field creates an `override_contradiction` review item instead of silently overwriting the manual value or silently ignoring the new evidence. New C.d writes use `mode = review_protected`; C.i still owns the full post-ingest contradiction service and review queue lifecycle. |
