...
| C.g | New project creation flow | `done` | 2026-04-27: Enabled `[+ New project]` in Pipeline. The modal requires canonical address, market, and jurisdiction, with optional project name/city/county/ZIP. Next.js submits through FastAPI `POST /projects`; the API normalizes the address, runs the existing conservative matcher against the normalized address, and returns duplicate candidates before creating unless `force_create=true`. Confirmed creates write a `projects` row, initial Proposed `status_history`, edit metadata, and a `change_log` audit row, then the UI opens Project Detail for the new project. Duplicate handling offers Open existing or Create anyway; true merge into an existing project remains out of scope. Geocoding, creation-time identifiers/source evidence, and DB-level duplicate locking remain future work. Verified with `npm run typecheck`, `npm run lint`, `npm run build`, scoped `ruff check`, `python -m pytest tests/test_project_creation_api.py tests/test_api_scaffold.py -q`, and `python -m pytest -q`. |
