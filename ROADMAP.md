...
| 2026-04-27 | Relationship links are create and note-update only in C.f | C.f brings Project Detail relationship linking online without adding destructive relationship operations yet. Project searches remain read-only Supabase server-action queries, while link creation and non-empty note updates write through FastAPI using the existing `project_relationships` table and emit `change_log` audit rows. Direct authenticated PostgREST writes remain blocked by RLS; unlink/retype behavior and explicit note clearing wait for a later relationship maintenance step. |
