...
| C.d | Inline editing on Core fields (Evidence-derived) | `done` | 2026-04-27: Implemented FastAPI set/clear override endpoints for evidence-derived Core fields (`pipeline_status`, unit counts, `developer`, `product_type`, `age_restriction`, `date_delivery`). Writes go through `researcher_overrides` with Supabase user IDs, keep legacy JSONB synchronized, re-run `resolve_project(apply=True)`, update project edit metadata, and write `change_log` rows with `researcher_override`. Project Detail Core rows now show an inline edit control and override info cue; Vercel preview server actions remain blocked by default unless `ENABLE_PREVIEW_WRITES=true` for an approved session. Contradiction review-item generation remains C.i. |
