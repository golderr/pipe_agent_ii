...
| C.5 | Python API (FastAPI) for write operations | `not_started` | **Decided 2026-04-23 — build a FastAPI service over `review_workflow.py`; do NOT port logic to TS Edge Functions or PL/pgSQL.** Deploy on Render alongside existing Python services. Endpoints: `/review/accept`, `/review/reject`, `/review/defer`, `/override/set`, `/override/clear` — thin wrappers over existing `review_workflow.py` functions. Auth: verify Supabase JWT against Supabase JWKS, pass user_id as the `set_by` / actor arg to workflow functions. Frontend uses `NEXT_PUBLIC_API_URL`. Reads stay direct via Supabase PostgREST with RLS; writes only through the API. Critical: RLS policies must ship with B.1 — read tables open to authenticated role, mutable tables (`researcher_override`, review decision tables, any StatusHistory/ChangeLog writes) locked to service role only. Rationale in Decision Log. |
...
| E.3a | Developer registry pollution hardening | `not_started` | Audit high-alias canonical rows before future apply cycles, tighten fuzzy matching and token-overlap guards where needed, and keep polluted canonical targets from reforming. Phase A fixed `Category` plus the observed `Capital` / `Investment` collisions, but this needs a durable follow-up pass. |
