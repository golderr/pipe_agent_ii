...
| A.6 | Review resolution discrepancies, tune if needed | `in_progress` | 2026-04-23: Validation found and fixed three systematic issues before rerun: future CoStar delivery dates no longer act as row-level freshness for unrelated fields, Stalled/Inactive transitions are manual-review only, and the polluted `Category` developer registry row is ignored; fuzzy-review developer suggestions are now flagged without becoming resolved values. Remaining review set: 9 status changes, 27 unit changes, 128 developer changes, and 253 delivery-date changes. |
| A.7 | Run `canonicalize-developers --market los_angeles` (shadow mode) | `done` | 2026-04-23: Shadow run completed with no writes. Scanned 319 registry rows and 967 projects with non-null developer values; would change 0 project values in this mode. Match summary: 1,134 exact, 2 fuzzy-auto, 13 fuzzy-review, 136 new registry-entry candidates. Review fuzzy/new candidates before any `--apply`. |
