...
| C.tail.11 | Backend decision-card consolidation | `not_started` | Phase D runway — must land before D.5. Promote `field_name` and `winning_evidence_id` to schema columns on `review_items` with a partial unique index on `(project_id, field_name, item_type) WHERE state IN ('open', 'staged')`. Modify ingest path in [src/tcg_pipeline/db/collect.py](src/tcg_pipeline/db/collect.py) to upsert against active items for `status_change`/`field_change` only, append evidence IDs to `payload.evidence_ids`, refresh `winning_evidence_id`, and invalidate-then-create on `proposed_value` flip. Reshape `override_contradiction` payload in [src/tcg_pipeline/review/contradictions.py](src/tcg_pipeline/review/contradictions.py) to compute and store the supporting-evidence set (currently only tracks dissenting candidate). Discovery items (`new_candidate`, `possible_match`) are explicitly out of scope. Keep `source_run_id` and `match_confidence` columns nullable as "latest representative." Backfill script `scripts/collapse_duplicate_review_items.py` aborts by default if any duplicate group contains a staged decision; `--migrate-staged` flag picks the staged item as survivor. Required reading: [docs/specs/review_decision_cards.md](docs/specs/review_decision_cards.md), [docs/specs/review_workflow.md](docs/specs/review_workflow.md) §2.3-§2.4. |
| C.tail.12 | Decision-card UI on Review Queue | `not_started` | Phase D runway. Depends on C.tail.11. Rework `ReviewItemRow` in [app/(app)/review/review-queue-client.tsx](app/(app)/review/review-queue-client.tsx) into a single decision card per `(project, field, item_type)` with one action button group and two independent collapsible Supporting/Against sections — counts visible when collapsed, top 2-3 evidence rows with inline summary when expanded. Server-side snippet rendering only: queue API returns precomputed inline evidence summary strings (new `summary_line()` per renderer in [src/tcg_pipeline/review/snippets.py](src/tcg_pipeline/review/snippets.py)); full snippets lazy-load on detail page via existing `GET /evidence/{id}/snippet?field=...`. Highlight `winning_evidence_id` in Supporting. Update [app/(app)/review/[itemId]/page.tsx](app/(app)/review/[itemId]/page.tsx) detail view to match. Add `supportingEvidenceForItem`, `dissentingEvidenceForItem`, `winningEvidenceForItem` helpers in [lib/review/payload.ts](lib/review/payload.ts) with Vitest coverage. Required reading: [docs/specs/review_decision_cards.md](docs/specs/review_decision_cards.md). |
...
| Step | Task | Status | Notes |
|------|------|--------|-------|
| D.1 | News research schema + dependency install | `not_started` | Two Alembic migrations per `news_research_design.md` §4.15: news content tables (`news_sources`, `news_articles`, `news_extractions`, `news_project_references`, `news_extraction_costs`, `news_cost_caps`, `news_signal_flag_registry`, `service_credentials`, `system_alerts`, `worker_heartbeats`, `service_credential_validations`, `news_admin_actions`) + summary views + `news_summary_reader` role; then `scrape_jobs` extension (`kind`, `target_payload`, nullable `jurisdiction_id`, narrowed unique index) + `evidence.superseded_at`. Add `bizjournals_la`, `news_paste_a_link`, `news_backfill`, `news_reextraction` → `news_article` to `LOGICAL_SOURCE_TYPE_BY_SOURCE_NAME` per §5.1. Add Phase D pyproject.toml deps (anthropic, playwright, trafilatura, dateparser, croniter, pyahocorasick) per §27. |
| D.W | Extend C.tail.1 RQ worker for news job kinds | `not_started` | Register `news_paste_a_link`, `news_scrape`, `news_reextract`, `news_backfill_chunk` task functions in `src/tcg_pipeline/workers/news_jobs.py`. Add daemon-thread heartbeat writer (per §12.6 — independent of task lifecycle). Add side-port `/healthz` for Render. Add in-process scheduler tick gated by `NEWS_SCHEDULER_LEADER=true` env. `system_alerts` dispatch hooks. Depends on D.1. |
| D.7a | Paste-a-link minimal vertical (Pass 0 only) | `not_started` | FastAPI `POST /research/articles` writes a `scrape_jobs(kind='news_paste_a_link')` row and enqueues into RQ; worker fetches via httpx + trafilatura; Pass 0 (ingest) only; admin view `/research/articles/{id}` shows article + body + metadata. Depends on D.W. |
| D.3a | Pass 1 structural extractor library | `not_started` | The 13 extractors from `news_research_design.md` §7.3 — unit_count, address, case/permit/APN, dates, status/delivery/product/age phrases, developer/project dictionary scans (Aho-Corasick). Wired automatically after Pass 0; populates `news_articles.structural_signals`. Depends on D.7a. |
| D.3b | Pass 2a triage (Haiku 4.5) | `not_started` | Broad-net triage prompt per §8.3 ("about real estate? cast a wide net"). Structured output + cost tracking + `news_extraction_costs` rollup writes via the advisory-lock pattern in §13.3. Depends on D.3a. |
| D.3c | Pass 2b extraction (Opus 4.7) | `not_started` | Full extraction prompt per §8.4 with glossary cache + signal-flag registry + structural-signals input. Output schema per §8.5. Materially-changed gating wired but not exercised. Depends on D.3b. |
| D.3d | Extraction-time Pass 3a re-extraction | `not_started` | Conflict / low-confidence / parse-error / refusal triggers per §8.7. Match-triggered Pass 3b ships in D.4. Depends on D.3c. |
| D.4 | Article matcher + integrator + Pass 3b | `not_started` | `news_matcher` Tier 1/2/3/4 from §9. Match-triggered Pass 3b on `new_candidate`. Evidence integrator with **post-apply review semantics** per §10 (one `STATUS_CHANGE` per changed field per §10.7.1 — single-element `payload.changes` list). SourceRun creation per §10.3. Connect to existing `_link_orphan_evidence` for orphan accept paths. Depends on D.3d. **Blocked on C.a.1** — article NLP will introduce many new developer names, so registry pollution hardening must land first or the Phase A cleanup will silently reform. |
...
| D.7b | Paste-a-link UI + article admin view | `not_started` | Next.js paste-a-link form. Article admin view (admin-only via `/research/articles/{id}`). Depends on D.5. |
| D.2 | BizJournals fetcher + cookie-injection auth | `not_started` | `BizJournalsCollector.discover_urls` + `fetch_article` per §6.1. CLI `tcg-pipeline news auth-bizjournals` per §16.1 — opens visible Chromium for manual login, dumps storageState to encrypted `service_credentials` row. Session-validity detection during fetch (GET-based per §16.2). Depends on D.7b — BizJournals comes after the rest of the pipeline is trusted. |
| D.6 | Scheduled scrapes via in-process scheduler | `not_started` | `news_sources.schedule_cron` evaluated by the scheduler tick (one worker instance flagged `NEWS_SCHEDULER_LEADER=true`). `last_scrape_started_at` derived from `source_runs` per §12.5 — no schema column. Depends on D.2. |
| D.M | Monitoring, alerting, cost-cap admin UI | `not_started` | SMTP email dispatcher to `ng@theconcordgroup.com`. `system_alerts` reader for the Coverage banner. News ops admin tile in Coverage. Cost-cap warn ($25) + hard ($35) + manual bump UI per §13.3. Depends on D.6. |
...
| D.B | 8-week BizJournals backfill | `not_started` | `tcg-pipeline news backfill --source bizjournals_la --since <8wk>`. ~200 URLs, ~$10 cost per §17.3. Observable via Coverage queue depth. Depends on D.6, D.M. |
