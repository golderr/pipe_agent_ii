...
| Step | Task | Status | Notes |
|------|------|--------|-------|
| B.0 | **Prerequisite:** take a production DB snapshot before schema migrations | `not_started` | Required immediately before B.0b/B.0c. Use Supabase point-in-time backup if available, or `pg_dump` schema+data. This is the recovery point if market/jurisdiction backfill logic is wrong. |
| B.0a | **Prerequisite:** complete the field inventory audit | `not_started` | Verify `docs/specs/field_inventory.md` against the live schema. Classify any post-scaffold fields. Auditor logs the result in the doc's Audit Log section. Required before any field-level edit UX can be designed. |
| B.0b | **Prerequisite:** data model migrations for jurisdictions + markets | `not_started` | Per `docs/specs/data_model_changes.md` §2 and §3. Create `markets` and `jurisdictions` tables with stable slugs, backfill from existing project slugs/market config, and seed initial LA coverage. Existing `projects.market` / `projects.jurisdiction` string columns remain in place until all CLI, collector, matcher, and UI reads have migrated. |
| B.0c | **Prerequisite:** source_runs expansion + source_registrations table | `not_started` | Per `docs/specs/data_model_changes.md` §4. Adds per-jurisdiction scoping needed for Coverage freshness display. Seed `source_registrations` from `config/markets/los_angeles.yaml`. |
| B.1 | Next.js scaffolding + Supabase + shadcn + auth + nav shell + one live read path | `not_started` | Next.js (app router), TypeScript, Tailwind, shadcn/ui, Supabase JS client + Supabase Auth (magic-link), email allowlist, unauthenticated redirect to `/login`, Mapbox GL JS via `react-map-gl`, cmdk, TanStack Query, Zustand, Lucide icons. Deploy to Vercel. Per-surface URL routing: `/coverage`, `/review`, `/pipeline`, `/dashboard`. RLS policies provisioned on day one — authenticated read on project/evidence/resolution_log/coverage tables; mutation tables locked to service role. B.1 must prove one end-to-end read path: an authenticated user loads `/coverage`, reads jurisdiction rows through Supabase PostgREST + RLS, and renders them. This `/coverage` route is deliberately minimal/unstyled in B.1; the full Coverage table, filters, pins, and source-detail UI are B.2. |
| B.2 | Coverage view | `not_started` | List of jurisdictions with columns per `docs/specs/ui_requirements.md` §3. Filters, pinning, optional columns via column picker. Read-only in Phase B — Refresh and Upload buttons arrive in Phase C. |
| B.3 | Pipeline list view (table + map) | `not_started` | Per `docs/specs/ui_requirements.md` §7. Dense table with keyboard nav, filter sidebar, hover preview. Map toggle using Mapbox GL, filter-aware, status-colored pins, clustering. `⌘K` command palette for project search. Sticky filter context. Saved views in local storage. |
| B.4 | Project Detail — Snapshot tab | `not_started` | Per `docs/specs/ui_requirements.md` §6. Four-section layout (Core / Identity / Notes / Relationships). Read-only display of all fields with source badges and three highlight states. Hover-evidence popovers per §10. |
| B.5 | Project Detail — Evidence tab | `not_started` | Per `docs/specs/ui_requirements.md` §6.7. Chronological, month-divided, collapsible rows with raw-data expand. Snippet rendering per source type (renderer functions implemented in C.b, scaffolded in B.5 with generic fallback). |
| B.6 | Project Detail — Resolution + Changes + Overrides tabs | `not_started` | Resolution tab shows rule-per-field, confidence, alternatives considered. Changes tab shows ChangeLog. Overrides tab shows active and superseded researcher overrides. All read-only in Phase B. |
| B.7 | Dashboard (5 tiles) | `not_started` | Per `docs/specs/ui_requirements.md` §18. Needs-attention, stalled candidates, contradictions, pipeline by status, recent activity. Each tile = single number + mini-viz + click-through. Ships last in Phase B. |
...
| Step | Task | Status | Notes |
|------|------|--------|-------|
| C.a | FastAPI backend scaffolding | `not_started` | **Decided 2026-04-23 — FastAPI over `review_workflow.py`.** Deploy as a Render service. Supabase JWT verification against Supabase JWKS. Endpoints per `docs/specs/review_workflow.md` §6. RLS on every table the UI reads; mutable tables locked to service role. Health check, logging, tracing. |
| C.b | Source snippet renderers per source_type | `not_started` | Per `docs/specs/review_workflow.md` §7. One function per source. Produces `SnippetPayload` used by the UI hover popovers. News article renderer stores highlights with the evidence row (for Phase D). |
| C.c | Promote `researcher_override` to a table | `not_started` | Per `docs/specs/data_model_changes.md` §6.4. Migration: extract JSONB overrides into rows in `researcher_overrides` table. Resolution engine updated to read from the table. Legacy JSONB column dropped after verification. |
...
| Step | Task | Status | Notes |
|------|------|--------|-------|
| C.h | `ReviewItem` / `ReviewDecision` staged/committed state machine | `not_started` | Per `docs/specs/data_model_changes.md` §5 and `docs/specs/review_workflow.md` §4. Schema migration adds state columns. Refactor `review_workflow.py` into decision layer (staging) + commit layer (transactional apply). |
| C.i | Contradiction detection service | `not_started` | Per `docs/specs/review_workflow.md` §5 and `docs/specs/EVIDENCE_LAYER_DECISIONS.md` §22. Runs after evidence ingest and after resolve_project; generates `override_contradiction` review items with at-minimum MEDIUM priority. |
...
| C.l | Coverage — scrape kickoff + CoStar upload | `not_started` | Per `docs/specs/ui_requirements.md` §3.5 and `docs/specs/data_model_changes.md` §8. Scrape jobs queue (RQ on Render). CoStar upload as file picker. Progress polling via `/scrape_jobs/{id}`. |
...
| E.3a | Developer registry + apply-script hardening | `not_started` | Three consolidated follow-ups from Phase A. **(1) Long-tail pollution audit:** Phase A cleaned `Category` plus the observed `Capital` / `Investment` / `Development` collisions via live 195-alias delete + token-overlap guard, but did not sweep every high-alias canonical row. Run a full audit of canonical rows with `>=3` aliases against the token-overlap guard and delete anything that now fails the guard. **(2) Architecture-firm override is name-based, not address-scoped:** `scripts/apply_phase_a_decisions.py:ARCHITECTURE_FIRM_RAW_VALUES` holds current developer for *any* row where raw = `MVE + Partners` or `Three 6Ixty`. Refactor to a list of `(project_id, raw_value, expected_current_developer)` tuples — or accept a per-row override CSV — before the script is reused. **(3) Registry-entry emission guard:** confirm the token-overlap guard also runs on *new* registry insertions during collector ingestion, not only during canonicalize-apply, so new polluted canonical rows can't form. **Must land before:** Phase D.4 (news-article NLP will introduce many new developer names at scale) and Phase H.6 (first Phase-A-style apply cycle for a new market — i.e., SM). |
...
| H.6 | Seed SM with CoStar + run full cycle | `not_started` | **Blocked on E.3a** — the Phase A apply script (`scripts/apply_phase_a_decisions.py`) currently hard-codes architecture-firm overrides by raw_value only. Before reusing it for SM, refactor to address-scoped or per-row overrides per E.3a item (2). |
