...
| 2026-04-25 | Use MapLibre and non-OSM public tiles for the Pipeline map | Mapbox GL JS v3 introduces token/licensing/telemetry concerns for this tokenless internal map, and OpenStreetMap's public tile service is not intended for routine app tile traffic. Pipeline now uses `react-map-gl/maplibre` + `maplibre-gl` and CARTO light raster tiles. Revisit paid/self-hosted tiles before broader external usage. |
