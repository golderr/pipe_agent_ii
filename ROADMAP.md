...
| C.f | Relationship picker UI | `done` | 2026-04-27: Implemented Project Detail relationship picker in the Relationships Snapshot section. The picker searches existing projects by name/address through a Next.js server action, then writes create-only links through FastAPI `POST /projects/{project_id}/relationship` into `project_relationships` with audit `change_log` rows. Supported relationship types are `phase`, `master_plan`, `counterpart`, `duplicate`, and `supersedes`; duplicate submissions are idempotent. Incoming and outgoing links are shown on Project Detail. Relationship unlink/retype remains out of scope for C.f. Verified with `npm run typecheck`, `npm run lint`, `npm run build`, scoped `ruff check`, `python -m pytest tests/test_project_relationship_api.py tests/test_api_scaffold.py -q`, and `python -m pytest -q`. |
...
| 2026-04-27 | Relationship links are create-only in C.f | C.f brings Project Detail relationship linking online without adding destructive relationship operations yet. Project searches remain read-only Supabase server-action queries, while link creation writes through FastAPI using the existing `project_relationships` table and emits `change_log` audit rows. Direct authenticated PostgREST writes remain blocked by RLS; unlink/retype behavior waits for a later relationship maintenance step. |
