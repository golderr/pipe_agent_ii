...
from `review_items.payload.evidence_ids`. Agent rows populate
`evidence_summaries[]` from `agent_runs.evidence_consulted` when a consulted
entry resolves to an evidence UUID, source record, or news article ID. Semantic
rows populate `evidence_summaries[]` from the Pass 2b
`news_project_references` row that the interpretation cites, using
`matched_evidence_id` or the accepted news-evidence source record. Raw
resolution evidence IDs, review-item evidence IDs, semantic reference metadata,
and agent consulted entries are preserved in `detail`. Summary hydration is
capped at five evidence rows per Activity row to keep feed payloads bounded.
Agent consulted-evidence summaries include a `role` value when the agent
supplied one (`primary`, `comparison`, etc.).
