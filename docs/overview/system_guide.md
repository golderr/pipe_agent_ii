...
1. Ship pre-cycle-1 Review Queue UX prep — scoped from the 2026-05-13 researcher walkthrough on real organic data. Four items: resolver-level suppression of benign LADBS follow-on permits, card source-detail rendering for permit + CoStar regression cards, 3-field Current/Evidence/Result model with simplified Confirm/Defer/Detail action buttons and auto-advance, and narrative descriptiveness (name specific permit types + news sources). ~3-4 days of focused work. See `ROADMAP.md` "Cross-cutting: Review Queue UX (AGENT.reset cycle 1 prep)"
