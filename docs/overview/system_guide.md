...
- **What's next** — pre-cycle-1 Review Queue UX prep (scoped from the 2026-05-13 researcher walkthrough: resolver suppression of benign LADBS follow-on permits, richer card source-detail rendering, 3-field Current/Evidence/Result review model with simplified action buttons, narrative descriptiveness, duplicate-prevention table for new candidates + possible matches, building-height/stories Project field), then `AGENT.reset` (the controlled production database rebuild that establishes the post-stabilization baseline). Permit cutover completed 2026-05-13 with a clean 25-row LADBS smoke ($0.063/agent run); `AGENT.gates` per-profile kill switches shipped 2026-05-13 so news and permit incidents can be killed independently. Then Santa Monica
...
1. Ship pre-cycle-1 Review Queue UX prep — scoped from the 2026-05-13 researcher walkthrough on real organic data. Six items: resolver-level suppression of benign LADBS follow-on permits, card source-detail rendering for permit + CoStar regression cards, 3-field Current/Evidence/Result model with simplified Confirm/Defer/Detail action buttons and auto-advance, narrative descriptiveness (name specific permit types + news sources), the duplicate-prevention table for new candidates + possible matches (new Discovery tab with subject-row + candidate-table layout, three-layer retrieval, cell-level overlap highlighting, click-to-view map popup, match-with-deltas, live updates), and the new building-height/stories Project field. ~2.5-3 weeks of focused work. See `ROADMAP.md` "Cross-cutting: Review Queue UX (AGENT.reset cycle 1 prep)"
