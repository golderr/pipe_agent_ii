...
`news_sources.schedule_cron` and `news_sources.schedule_timezone` are read every 60 seconds by the scheduler tick. Cron expressions are parsed via `croniter` in the source's IANA timezone (for `urbanize_la`, `America/Los_Angeles`) so the intended local fire time survives DST changes. When a scheduled tick fires, the scheduler applies deterministic per-source jitter (`NEWS_SCHEDULER_JITTER_SECONDS`, overrideable by `news_sources.config.schedule_jitter_seconds`), inserts a `scrape_jobs` row of kind `news_scrape` with `trigger_type='scheduled'`, then enqueues the matching RQ task once the jittered due time arrives. The initial Urbanize candidate is `30 7 * * *`, roughly 75-90 minutes after the observed RSS publication window.
