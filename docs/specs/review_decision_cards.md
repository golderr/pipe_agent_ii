...
1. Inline summary string returned precomputed in the queue API, generated server-side from the existing `render_snippet(...).summary` renderer output in `snippets.py`.
...
1. **Inline summary format.** Server returns one summary line per evidence row. The C.tail.12 implementation reuses each renderer's existing `render_snippet(...).summary` in [src/tcg_pipeline/review/snippets.py](../../src/tcg_pipeline/review/snippets.py) instead of adding a separate renderer method.
