...
**Prerequisite status:** This is a **Phase B prerequisite**. The classifications below are pre-filled from `ARCHITECTURE.md` §3d and the schema in `src/tcg_pipeline/db/models.py`. The dev executing Phase B work must verify each classification against the live data model and update where it has drifted.
...
### 3.5 Researcher override storage
...
### 3.6 Additional identifiers and relationships
...
- **Source-populated direct**: ~14 fields (rent/sale, physical attributes, unit mix, CoStar submarket, owner, zoning)
...
The following need confirmation during the audit:

1. **`raw_addresses[]` vs `canonical_address` edit path.** My classification puts `raw_addresses[]` as researcher-authored (editable) and `canonical_address` as computed (derived from the address normalizer). Confirm this matches the intent — i.e., users edit addresses as strings, and the normalizer runs server-side on save.
2. **`delivery_year` vs `date_delivery`.** `ARCHITECTURE.md` mentions a `delivery_year` column separately from `date_delivery`. If both exist, confirm class assignments and whether both are UI-editable.
3. **Source-populated direct exact list.** MVP behavior is decided: read-only. During audit, verify the exact set of direct-source fields against `models.py`, `ingesters/costar.py`, `ingesters/pipedream.py`, and `source_adapters/*`.
4. **`inclusion_in_analysis` / `inclusion_in_exhibit`.** Confirm these fields exist in the schema. If not yet implemented, they are a Phase C.m task.
5. **Planner fields persistence.** Given <1% population in Pipedream, confirm we still want to preserve these or drop them. If dropped, reclassify as "not migrated."

Fill in the auditor's findings in an "Audit Log" section below when complete.
