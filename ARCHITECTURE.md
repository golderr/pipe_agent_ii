...
- [ ] **UC not-found failure-mode distribution:** `cpkv-aajs` and `3f9m-afei` are now implemented. The next audit pass should rerun the full LADBS family audit and classify the remaining `Under Construction` not-found bucket into matching gaps, stale seeded statuses, source incompleteness, and source-chaining opportunities.
