...
| 2.9 | Implement Socrata incremental cursor + overlap strategy | `not_started` | Collector-level enhancement: use Socrata system timestamps (`:updated_at`) for loss-resistant incremental syncs and deterministic replay order. |
| 2.10 | Add source-row change metadata + reconciliation reporting | `not_started` | Track source-system row identity, row hashes, last-seen timestamps, and reconciliation metrics before building additional LADBS adapters. |
