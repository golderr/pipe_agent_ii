...
| 2.9 | Implement Socrata incremental cursor + overlap strategy | `done` | `src/tcg_pipeline/collectors/socrata.py` now requests Socrata system fields, applies mode-specific ordering (`preview` newest-first, production oldest-first), and supports `:updated_at`-bounded incremental pulls. `collect-source --mode incremental` derives its lower bound from the latest persisted `SourceRun` cursor metadata and re-reads an overlap window. |
...
| Step | Task | Status | Notes |
|------|------|--------|-------|
| 3.0 | Configure LADBS new housing sibling source (`cpkv-aajs`) | `done` | `src/tcg_pipeline/source_adapters/ladbs.py` now maps `cpkv-aajs` rows into `RawRecord`s with permit/APN identifiers, unit detail, and `location_1` coordinates. Live preview verified on 2026-04-16. |
...
- [ ] **UC not-found failure-mode distribution:** After `cpkv-aajs` and `3f9m-afei` are implemented, rerun the LADBS audit and classify the remaining `Under Construction` not-found bucket into matching gaps, stale seeded statuses, source incompleteness, and source-chaining opportunities.
