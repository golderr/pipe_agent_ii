...
| 2.6 | Run first LADBS collection + match/diff cycle | `done` | Live preview verified the endpoint and adapter output on 2026-04-16, followed by a committed `collect-source ladbs_permits --market los_angeles --limit 100` run that produced 100 new-candidate review items against the active database. |
| 2.7 | Write LADBS Socrata completeness spec + recall audit plan | `done` | Treat LADBS as a source bundle rather than a single endpoint. Working docs now live under `docs/source_specs/` and `docs/audits/`. |
| 2.8 | Run recall audit against seeded LA projects | `done` | Completed on 2026-04-16. Results documented in `docs/audits/ladbs_recall_audit_results_2026-04-16.md`: the current `permit_type='Bldg-New'` filter is too narrow to serve as the full LADBS lifecycle entry point, especially for `Under Construction`. |
| 2.9 | Implement Socrata incremental cursor + overlap strategy | `done` | `src/tcg_pipeline/collectors/socrata.py` now requests Socrata system fields, applies mode-specific ordering (`preview` newest-first, production oldest-first), and supports `:updated_at`-bounded incremental pulls. `collect-source --mode incremental` derives its lower bound from the latest persisted `SourceRun` cursor metadata and re-reads an overlap window. Operator note: the current persisted `ladbs_permits` cursor was seeded from a bounded oldest-first slice on 2026-04-16 and is not yet a production-meaningful high-water mark; do not rely on default incremental behavior for that source until it has been backfilled or explicitly re-anchored. |
| 2.10 | Add source-row change metadata + reconciliation reporting | `in_progress` | Current-row metadata is now persisted on `ProjectSourceRecord` (`source_row_id`, source timestamps, row hash) and `SourceRun` (collection mode plus source updated-at bounds). Full reconciliation reporting and explicit source-disappearance handling are still pending. |
| 2.11 | Review results, tune matching thresholds | `not_started` | Iterative |
...
| 3.0c | Audit the remaining LADBS-family `Under Construction` miss bucket and profile inspections | `done` | Closed on 2026-04-17 by `docs/audits/ladbs_socrata_coverage_2026-04-17.md` plus `docs/audits/ladbs_socrata_rewire_verification_2026-04-17.md`. The remaining miss bucket was mostly a frozen-source coverage problem, not a matcher-only problem; `9w5z-rg2h` is now the inspections source with conservative direct-UC gating. |
| 3.0d | Stratified triage sample of the UC not-found bucket | `done` | `docs/audits/ladbs_uc_triage_sample_2026-04-16.md` triages a deterministic n=15 sample from the 71-project UC not-found bucket: `matcher-fixable` 3, `seed-stale` 1, `genuine-ladbs-gap` 8, `cant-tell` 3 (directional only, n=15). Primary recommendation is a narrowly targeted seed-cleanup pass on the full 71; secondary is an APN-first matcher improvement for `cpkv-aajs`/`3f9m-afei` (Hudson at 640 S St Andrews Pl is findable in `cpkv-aajs` by APN today but indexed under ZIP 90010 vs seed ZIP 90005). The LA Case Reports / DCP adapter at Step 3.2 remains gated on the post-cleanup residual gap. Superseded by `3.0e` below: the "genuine-gap" bucket is mostly a Socrata-coverage artifact, not a true LADBS absence. |
| 3.0e | Diagnose LADBS Socrata coverage / completeness of `hbkd-qubn` and `cpkv-aajs` | `done` | `docs/audits/ladbs_socrata_coverage_2026-04-17.md` establishes that `hbkd-qubn` and `cpkv-aajs` were frozen by the publisher on `2023-05-22T09:33:30.736Z` and have received zero row updates since; 13/13 Pipedream-cited `Bldg-New` permits missing from our bundle are present in the replacement dataset `pi9x-tg5x` ("Building Permits Issued from 2020 to Present (N)", 385k rows, `:updated_at` 2026-04-13). A 10-project CoStar-only spot-check found 6/10 recover via `Bldg-New` in `pi9x-tg5x`, 1/10 via `Bldg-Alter/Repair` adaptive reuse (Mama Shelter DTLA), 3/10 remain unresolved (wrong-street seeds or genuinely pre-permit). Combined extrapolation across the 71 bucket: ~50 recoverable via adapter rewire, ~21 residual (wrong-jurisdiction + seed errors + genuinely absent). Inspections moved to `9w5z-rg2h` (live, 11.3M rows, now includes `address` — replaces the sparse `2w4b-a48u`); `ydma-y4hd` removed from catalog; `3f9m-afei` remains live. Next-step priority: rewire `ladbs_permits`/`ladbs_permit_activity`/`ladbs_new_housing` to `pi9x-tg5x` before any further seed cleanup or matcher work; evaluate `9w5z-rg2h` as a new inspections-based UC evidence source; then a focused seed-cleanup pass on the ~21 residual should bring total recovery to ~55-58 of 71. |
| 3.0f | Rewire active LADBS Socrata sources to the live replacement datasets | `done` | `config/markets/los_angeles.yaml` now points `ladbs_permits` and `ladbs_permit_activity` at `pi9x-tg5x`, retires active `ladbs_new_housing`, adds `ladbs_inspections` on `9w5z-rg2h`, and keeps `ladbs_cofo` unchanged. Verification in `docs/audits/ladbs_socrata_rewire_verification_2026-04-17.md` landed at `15 / 15` Pipedream permit recovery, `12 / 15` qualifying inspections, and `7 / 10` clean CoStar recovery. |
...
| 2026-04-16 | Treat Socrata completeness as four separate concerns: row, field, change, and lifecycle coverage | "Did we pull the row?" and "did we infer the right status from the right source family?" are different failure modes. Architecture and QA must address them separately instead of assuming one newer query solves all gaps. |
| 2026-04-16 | Treat LADBS as a bundle of related Socrata datasets, not one monolithic source | `hbkd-qubn`, `ydma-y4hd`, `cpkv-aajs`, `3f9m-afei`, and `2w4b-a48u` cover different parts of the project lifecycle with different field quality and authority. Completeness comes from coordinating the bundle, not from overloading one endpoint. |
| 2026-04-16 | Preview and production Socrata queries should use different shapes | Preview should surface recent, representative rows for human sanity-checking. Production should be deterministic and loss-resistant, using full backfills plus `:updated_at`-driven incremental syncs. |
| 2026-04-16 | Periodic full reconciliation is a required operating mode for Socrata sources | Reconciliation catches source-side corrections, row disappearance, and bugs in our own earlier filters or adapters. Without it, historical records remain processed under stale logic forever. |
| 2026-04-16 | Source-row disappearance should be tracked first, not auto-applied to canonical project state | If a Socrata row disappears between runs, treat it as a source-state event until repeated absence or stronger evidence justifies changing identifiers or project state. |
| 2026-04-16 | Keep `permit_type='Bldg-New'` as a narrow LADBS slice, but do not treat it as the full LADBS lifecycle answer | The first recall audit showed that `Bldg-New` has acceptable precision as a specific permit-evidence slice but weak recall for seeded `Under Construction` projects. Broader permit activity exists at many project addresses, but blindly widening to all permit types would mix materially different signals. |
...
| 2026-04-17 | Only recent, substantive LADBS inspections on active permits emit direct `Under Construction` evidence | `9w5z-rg2h` mixes real field progress with scheduled, cancelled, correction-only, and terminal-permit rows. The adapter now persists all inspection context but only emits `building_inspection_recorded` when the inspection is recent, the result is substantively positive, and the permit is still in an active in-progress status. |
| 2026-04-16 | The full LADBS bundle improves semantics more than recall rescue for the remaining UC gap | The full-bundle rerun in `docs/audits/ladbs_recall_audit_bundle_2026-04-16.md` shows broader `hbkd` activity remains the main recall lift, `cpkv-aajs` provides modest rescue plus enrichment, and `3f9m-afei` mainly adds direct `Complete` evidence rather than rescuing the unresolved `Under Construction` miss bucket. A follow-up identifier-based check found `0 / 75` of the remaining UC misses recoverable through existing permit/APN-based production matching, so the next investigation should focus on stale seed state and source-family absence rather than assuming an immediate matcher bug. |
...
| 2026-04-15 | Pipedream seed persistence runs in two passes: insert records first, then resolve staged relationships by `tcg_pipedream_id` | `ProjectRelationship` rows depend on generated project UUIDs and may point across multiple Pipedream workbooks, so persistence resolves links only after all imported projects and identifiers have been flushed. |
| 2026-04-15 | Pipedream import should warn on bad source values instead of silently defaulting | Invalid statuses, enum values, date parses, missing ProjectIDs, and suspicious abbreviated project identifiers are now captured as diagnostics and logged during import. |
...
- [x] **LADBS address geocoding:** **RESOLVED 2026-04-16.** `hbkd-qubn` still lacks visible coordinates/APN, but `cpkv-aajs` exposes `location_1` geometry plus assessor book/page/parcel that can be normalized into APN. Use `hbkd-qubn` for broad permit issuance/detail and `cpkv-aajs` as the current LADBS source of truth for APN/coordinate enrichment when present.
...
- [ ] **Pipedream relationship ID format:** `CorrP`, `PCPart`, and `RelP1-6` currently normalize abbreviated numeric IDs by zero-padding the suffix to 5 digits and log suspicious values (e.g. `23.1` → `23.10000`). Verify against a real workbook whether relationship fields always use the full 5-digit sequence or sometimes abbreviate it.
...
- [x] **LADBS permit recall audit:** **RESOLVED 2026-04-16.** The first audit is documented in `docs/audits/ladbs_recall_audit_results_2026-04-16.md`. `permit_type='Bldg-New'` is too narrow to serve as the full LADBS lifecycle strategy, especially for seeded `Under Construction` projects.
- [x] **Socrata row-state model:** **RESOLVED 2026-04-16.** The first completeness pass will use current-row metadata plus a row hash, not a full source-row history/version table. Revisit explicit version history only if researchers later need historical replay of source-row contents.
- [ ] **Source disappearance policy:** When a Socrata row vanishes from a later reconciliation, how many absent reconciliations or what additional evidence should be required before changing canonical identifiers or project state?
- [ ] **UC not-found failure-mode distribution:** The full LADBS bundle rerun is now documented in `docs/audits/ladbs_recall_audit_bundle_2026-04-16.md`. The remaining `Under Construction` not-found bucket is `75 / 224` projects. A follow-up identifier-based check found `0 / 75` recoverable through existing permit/APN-based production matching, `56 / 75` with identifiers but no current LADBS-bundle rows found by those identifiers, and `19 / 75` with no APN/permit identifier available. Next step is to classify the bucket into stale seeded statuses, source incompleteness, and source-chaining opportunities, with `2w4b-a48u` treated as a profiling target rather than an assumed adapter-ready source.
